...
Visual Context Analyzer (CodeLens) runs as an Electron overlay. TypeScript sources live in `src/` (main process, bridge, logging, analysis). Frontend markup is in `index.html`, with component styles in `styles/`. Local renderer vendor assets live in `vendor/`. Tests sit in `test/` alongside fixtures; compiled assets land in `dist/` and packaged builds in `release/`. Runtime logs rotate under `logs/`. Screenshots write to the OS temp directory under `codelens-screenshots/screenshot-{slot}-{timestamp}.png`; never check them in.

## Build, Test, and Development Commands
Use npm for package management and scripts; commit `package-lock.json` (required for CI's `npm ci`). Dependency lifecycle scripts are disabled by default via `.npmrc` (`ignore-scripts=true`) for supply-chain hygiene. After `npm install`, run `npm run setup` once — it fetches the Electron binary and wires the husky pre-commit hook. `npm run dev` starts the watch-driven development loop that recompiles TypeScript and restarts Electron. `npm run build` transpiles to `dist/`, while `npm start` builds and launches the desktop app. Type-check without emitting files via `npm run typescript-check`. Run the suite with `npm test`, add `npm run test:watch` for TDD, and prefer `npm run test:coverage` before major merges. Ship installers with `npm run package[-win|-linux|-all]` after a clean build. Note: adding a dependency that needs a postinstall (e.g. a native module) requires extending the `setup` script, since install scripts won't run automatically.
