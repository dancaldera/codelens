...
Use Bun for package management and scripts; do not create or commit `package-lock.json`. Use `bun install` to install dependencies. `bun run dev` starts the watch-driven development loop that recompiles TypeScript and restarts Electron. `bun run build` transpiles to `dist/`, while `bun start` builds and launches the desktop app. Type-check without emitting files via `bun run typescript-check`. Run the suite with `bun run test`, add `bun run test:watch` for TDD, and prefer `bun run test:coverage` before major merges. Ship installers with `bun run package[-win|-linux|-all]` after a clean build.
