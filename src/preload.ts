...
contextBridge.exposeInMainWorld('api', {
	...
	onScreenshotStatus: (callback: (status: string) => void) =>
		ipcRenderer.on('screenshot-status', (_e: IpcRendererEvent, status: string) => callback(status)),
	// Analysis result from main
	onAnalysisResult: (callback: (result: string) => void) =>
		ipcRenderer.on('analysis-result', (_e: IpcRendererEvent, result: string) => callback(result)),
	...
	onSubmitResult: (callback: (result: string) => void) =>
		ipcRenderer.on('submit-result', (_e: IpcRendererEvent, result: string) => callback(result)),
	...
	onScreenshotImage: (callback: (imageData: ScreenshotImageData) => void) =>
		ipcRenderer.on('screenshot-image', (_e: IpcRendererEvent, imageData: ScreenshotImageData) => callback(imageData)),
	// Clear screenshots
	...
	onGetPrompt: (callback: () => string) =>
		ipcRenderer.on('get-prompt', (_e: IpcRendererEvent) => {
